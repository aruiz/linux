 ...
 *   3. When the initramfs switch_root unmounts the overlay, EROFS closes the
 *      block devices.  The .release callback detects the last close and
 *      schedules initrd_blkdev_shutdown() on a workqueue.
 *   4. initrd_blkdev_shutdown()  — del_gendisk + put_disk on every device.
 *      When the final gendisk refcount drops, .free_disk returns the backing
...
static atomic_t initrd_blk_open_count = ATOMIC_INIT(0);
...
static void initrd_blkdev_shutdown(void)
{
	int i;
...
static void initrd_blk_shutdown_work(struct work_struct *work)
{
	initrd_blkdev_shutdown();
}

static DECLARE_WORK(initrd_blk_work, initrd_blk_shutdown_work);

static int initrd_blk_open(struct gendisk *disk, blk_mode_t mode)
{
	atomic_inc(&initrd_blk_open_count);
	return 0;
}

static void initrd_blk_release(struct gendisk *disk)
{
	if (atomic_dec_and_test(&initrd_blk_open_count))
		schedule_work(&initrd_blk_work);
}
...
static void initrd_blk_submit_bio(struct bio *bio)
{
	...
	bio_for_each_segment(bv, bio, iter) {
		...
		unsigned long avail = dev->size - min_t(loff_t, pos, dev->size);
		unsigned long len = min_t(unsigned long, bv.bv_len, avail);

		if (len)
			memcpy(dst, dev->data + pos, len);
		if (len < bv.bv_len)
...
static const struct block_device_operations initrd_blk_fops = {
	.owner = THIS_MODULE,
	.open = initrd_blk_open,
	.release = initrd_blk_release,
