...
provides a read-only, compressed root filesystem that is mounted via a
lightweight RAM block device backed by the initrd memory region, without
unpacking.
...
- **EROFS segments** are exposed as read-only RAM block devices and mounted
